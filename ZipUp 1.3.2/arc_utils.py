...
def open_zip(path: str, mode: str = 'r') -> Iterator[Tuple[zipfile.ZipFile, Optional[None]]]:
    ...
    # Choose backend: zipfile (standard) or pyzipper (if available and reading)
    ZipCls = zipfile.ZipFile
    if mode == 'r' and pyzipper is not None:
        ZipCls = pyzipper.AESZipFile  # supports traditional and AES-encrypted zip-files

    zf = ZipCls(fileobj, mode, compression=comp, **kwargs)
    try:
        yield zf, None
    finally:
        zf.close()
